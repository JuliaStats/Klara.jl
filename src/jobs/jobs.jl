...
function set_mcjob{M<:MCModel, S<:MCSampler, T<:MCTuner}(m::M, s::S, r::SerialMC, t::T, j::Symbol)
  mcjob::MCJob
  if j == :plain
...
set_mcjob{M<:MCModel, S<:MCSampler, T<:MCTuner}(m::M, s::S, r::SerialMC; tuner::T=VanillaMCTuner(), job::Symbol=:task) =
  set_mcjob(m, s, r, tuner, job)

### Constructors for setting up a vector of Monte Carlo jobs

function set_mcjob{M<:MCModel, S<:MCSampler, T<:MCTuner}(m::Vector{M}, s::Vector{S}, r::Vector{SerialMC}, t::Vector{T},
  j::Vector{Symbol})
  @assert length(m) == length(s) == length(r) == length(t) == length(j)
    "Number of models, samplers, runners, tuners and job types not equal."
  map(set_mcjob, m, s, r, t, j)
end

set_mcjob{M<:MCModel, S<:MCSampler, T<:MCTuner}(m::Vector{M}, s::Vector{S}, r::Vector{SerialMC};
  tuners::Vector{T}=fill(VanillaMCTuner(), length(m)), jobs::Vector{Symbol}=fill(:task, length(m))) =
  map(set_mcjob, m, s, r, tuners, jobs)
