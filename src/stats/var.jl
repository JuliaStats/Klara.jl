...
function var_imse(mcmc::DataFrame; maxlag::Int)
  ...
  for i = 1:npars
    acv[:, i] = acf(mcmc[:, i], 0:maxLag, actype="covariance")
  end
...
var_imse(mcmc::DataFrame) = var_imse(mcmc::DataFrame, maxlag=size(mcmc, 1)-1)

# Standard deviation using Geyer's IMSE
std_imse(mcmc::DataFrame; maxlag::Int) = sqrt(var_imse(mcmc::DataFrame, maxlag=maxlag))

std_imse(mcmc::DataFrame) = sqrt(var_imse(mcmc::DataFrame, maxlag=size(mcmc, 1)-1))

# Wrapper function for computing MCMC variance using various approaches
function var(mcmc::DataFrame; vtype::ASCIIString="imse")
  if vtype == "imse"
...
function std(mcmc::DataFrame; vtype::ASCIIString="imse")
  if vtype == "imse"
