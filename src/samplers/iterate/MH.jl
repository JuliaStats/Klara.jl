function codegen_iterate_mh(job::BasicMCJob)
  ...
  push!(body, :(_job.sstate.pstate.value = _job.sampler.randproposal(_job.pstate.value)))
  ...
  if job.sampler.symmetric
    ...
    push!(body, :(_job.sstate.ratio = (
      _job.sstate.pstate.logtarget
      +_job.sampler.logproposal(_job.sstate.pstate.value, _job.pstate.value)
      -_job.pstate.logtarget
      -_job.sampler.logproposal(_job.pstate.value, _job.sstate.pstate.value)
    )))
  end
