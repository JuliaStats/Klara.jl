abstract RobertsRosenthalMCTune <: MCTunerState

type UnvRobertsRosenthalMCTune <: RobertsRosenthalMCTune
  σ::Real # Standard deviation of underlying normal or truncated normal proposal
...
type MuvRobertsRosenthalMCTune <: RobertsRosenthalMCTune
  σ::RealVector # Standard deviation of underlying normal or truncated normal proposal
  δ::Real # Increase or decrease of σ at each batch
  ...
  function MuvRobertsRosenthalMCTune(
    ...
    for r in rate
      if !isnan(r)
        @assert 0 < r < 1 "Observed acceptance rate should be between 0 and 1"
      end
    end
    new(σ, δ, accepted, proposed, totproposed, rate)
  end
...
Base.show(io::IO, tuner::RobertsRosenthalMCTuner) =
  print(
    io, "RobertsRosenthalMCTuner: target rate = $(tuner.targetrate), period = $(tuner.period), verbose = $(tuner.verbose)"
  )

Base.writemime(io::IO, ::MIME"text/plain", tuner::RobertsRosenthalMCTuner) = show(io, tuner)
